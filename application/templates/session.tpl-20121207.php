            <?php if ($this->registrationStatus == 'success' ): ?>
                <p>
                    ...
                    <?php else: ?>
                    Votre <strong>demande d'inscription</strong> à la formation
                    <?php endif; ?>
                    ...
                    <?php if ($this->session->Formation->related_record->type_de_formation['value'] != 4): ?>
                    <span class="emphase">Vous allez recevoir un email de confirmation prochainement. ATTENTION, votre <em>inscription ne sera effective qu'après reception de cet email</em></span>.
                    <?php else: ?>
            ...
            <?php elseif ($this->registrationStatus == 'requested' ):?>
                <?php if ( ( ( empty($this->session->date) && $this->session->Formation->related_record->type_de_formation['value'] == 4 ) 
                        ...
                        || (!empty( $this->session->inscription_fin ) && strtotime( $this->session->inscription_fin ) > time() - 24 * 3600 ) )
                ...
                <?php else: ?>
                    <?php if ( isset( $this->registrationMessage ) ) : ?>
                        <p class="registrationMessage">
                            <strong style="color:red;"><?php echo $this->registrationMessage; ?></strong>
                        </p>
                    <?php endif; ?>
